...
public class StudyList {
	...
	public Item find(String stimulusOrResponse) {
		...
		for (int i = 0; i < flashCards.size(); i++) {
			...
			if (card.toString().toLowerCase()
					.contains(stimulusOrResponse.toLowerCase())) {
				return card;
			}
		}
		return null;
	...
	public void modify(Item item, String newStimulus, String newResponse) {
		// PROBLEM: This searches both stimulus and response, so could get false
		// positive
		if (newStimulus != item.getStimulus()
				&& (this.find(newStimulus) != null)) {
			throw new IllegalArgumentException();
		}
